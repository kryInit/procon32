...
void loop_move(Vec2<int> &pos, Direction dir, const Vec2<int> div_num) {
    pos.move(dir);
    pos.x = (pos.x+div_num.x) % div_num.x;
    pos.y = (pos.y+div_num.y) % div_num.y;
}

Vec2<int> get_pos(const OriginalPos orig_pos, const OriginalPositions& now_orig_pos, const Vec2<int> div_num) {
...
Path calc_path(const Pos s, const Pos g, const Vec2<int> border, const vector<Pos> &exclusions, const Vec2<int> div_num) {
    ...
    repo(i, border.y, div_num.y) rep(j,div_num.x) keep_out[i][j] = true;
    repo(j, border.x, div_num.x) rep(i, div_num.y) keep_out[i][j] = true;
    ...
    // いつもの
    reverse_priority_queue<ComparableData<int, pair<Pos, Direction>>> pq;
    pq.emplace(0, make_pair(s, Direction::U));
    while(!pq.empty()) {
        int dist = pq.top().eval_val;
